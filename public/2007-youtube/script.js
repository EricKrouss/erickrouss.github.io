...
const WATCH_CLASSIC_PLAYER_WIDTH = 450;
const WATCH_CLASSIC_VIDEO_HEIGHT = 339;
const WATCH_CLASSIC_CONTROLS_HEIGHT = 31;
const WATCH_CLASSIC_SIDE_WIDTH = 300;
...
function enforceWatchClassicWindowedBox() {
  ...
  if (layout) {
    ...
    layout.style.setProperty('column-gap', '15px', 'important');
  }
...
if (!isYouTubeMode) {
  myVideo.addEventListener('loadedmetadata', () => {
    ...
    endedButtons.style.display = 'none';
    myVideo.pause(); // Embedded desktop opens silently.
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (isYouTubeMode) {
    ...
    enterYouTubeMode(ytVideoId, ytStartAt).catch((err) => {
      ...
      myVideo.pause(); // Playback starts with the player controls.
    });
  } else {
    // Keep the original demo paused until the visitor presses Play.
    myVideo.pause(); // Playback starts with the player controls.
  }
  if (isWatchTheaterModeActive()) {
    enforceWatchTheaterWindowedBox();
  ...
  if (ytLoaderForm && !isWatchClassicLayout()) {
...
function ensureYouTubeAPI() {
  return new Promise((resolve, reject) => {
    ...
    const checkReady = () => {
      ...
      if (Date.now() - t0 > 10000) return reject(new Error('YouTube API unavailable'));
...
let latestYouTubeRequest = 0;
async function enterYouTubeMode(videoId, startAt = 0) {
  const request = ++latestYouTubeRequest;
  ...
  if (request !== latestYouTubeRequest) return;
  ...
  if (!ytPlayer) {
    ...
    ytPlayer = new YT.Player('ytContainer', {
      ...
      events: {
        ...
        onError: () => window.dispatchEvent(new CustomEvent('watch-playback-error', { detail: { id: ytPlayer?.getVideoData()?.video_id || videoId } })),
        onStateChange: (e) => {
