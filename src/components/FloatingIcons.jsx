...
const FloatingIcons = () => {
  ...
  // Generate an array of 20 floating icons with random properties.
  const bubbles = Array.from({ length: 10 }).map((_, i) => {
    const icon = icons[Math.floor(Math.random() * icons.length)];
    const left = Math.random() * 100; // random left position (percentage)
    const size = 20 + Math.random() * 30; // random size between 30px and 80px
    const delay = Math.random() * 5; // random delay between 0 and 5s
    // Duration between 20 and 40 seconds (adjust as desired)
